...
const Booking = () => {
    ...
    const [tempItem, setTempItem] = useState<{ service: string; tonnage: number; quantity: number }>({ service: '', tonnage: 1, quantity: 1 });
    ...
    const calculateItemPrice = (service: string, tons: number, qty: number) => {
        let base = 0;
    ...
    const addToCart = () => {
        ...
        const newItem: CartItem = { ...tempItem, price, id: Date.now() };
    ...
    return (
        <div className="container" style={{ overflowX: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '24px' }}>
                <button onClick={() => step > 1 ? prevStep() : navigate('/home')} style={{ background: 'none', padding: 0, border: 'none', cursor: 'pointer' }}>
                                ...
                                <div style={{ marginBottom: '16px' }}>
                                    <button
                                        onClick={() => setFormData({
                                            ...
                                            lat: Number(user.default_lat || 24.809065),
                                            lng: Number(user.default_lng || -107.394017)
                                        })}
                                        style={{
                                            padding: '8px 12px',
                                    ...
                                    <ul style={{
                                        ...
                                        {suggestions.map((s) => (
                                            <li
                                                ...
                                                onMouseEnter={(e) => (e.target as HTMLLIElement).style.backgroundColor = '#f5f5f5'}
                                                onMouseLeave={(e) => (e.target as HTMLLIElement).style.backgroundColor = 'white'}
                                            >
                                                {s.display_name}
                                            </li>
                                        ))}
